...
- **YouTube Transcripts:** `youtube_transcript_etl.py` — YouTube VTT auto-captions → transcripts.json. 44 meetings across 7 borough councils (5,518 moments). YouTube embed support on Transcripts page
...
- **Cabinet Command:** `cabinet_portfolios.json` → `savingsEngine.js` → Executive/DirectorateDashboard/DirectorateDetail/CabinetDashboard/PortfolioDetail pages. 5 directorates (matching LCC management structure) with evidence-backed savings, KPI→savings chains, 62 engine functions. 10 LCC portfolios with budget mapping, savings levers, statutory duties, governance routing. Prescriptive ACTION DIRECTIVES: DO/SAVE/LEGAL/ROUTE/HOW/EVIDENCE format. Deep Service Intelligence: SEND cost cascade (EHCP→placement→transport→tribunal), ASC demand model (demographics→care costs→market risk), cross-cutting intelligence (demand quantification, budget realism, inspection remediation, net fiscal trajectory). Spending Intelligence: 13-category classification, `useSpendingSummary` singleton hook, cross-platform spending access (Budgets, DOGE, Home, Executive, CrossCouncil, DirectorateDashboard, DirectorateDetail, PortfolioDetail). LCC-only (gated by `cabinet_portfolios` config flag).
...
| `src/hooks/useData.js` | Data fetching hook with AbortController cleanup, 30min TTL cache, retry, LRU eviction |
| `src/hooks/useClipboard.js` | Clipboard copy hook with fallback + "Copied!" feedback timer |
| `src/components/ui/ErrorState.jsx` | Shared error state component (used by 12+ pages) |
| `src/test/mocks.js` | Shared test mock data (council configs, useData helpers) |
| `src/test/setup-recharts.jsx` | Global Recharts mock for all vitest files |
...
| `src/**/*.test.{js,jsx}` | Unit tests: 3,020 tests across 55 files (vitest) |
...
| `burnley-council/scripts/youtube_transcript_etl.py` | YouTube VTT auto-captions → transcripts.json per council (44 meetings, 7 boroughs, 5,518 moments) |
...
| `transcripts.json` | youtube_transcript_etl.py / transcripts_aggregator.py | Meeting transcript moments: flagged quotes, scores, topics, speakers, clip metadata. LCC from Whisper/Mediasite, 7 boroughs from YouTube VTT |
...
- **Octavian (Clawdbot):** 3 bots (WhatsApp/Telegram/Mac) on vps-main via OpenClaw 2026.3.13. WA: Groq Scout 17B → 7 fallbacks. TG: Haiku 4.5 → 7 fallbacks. Config: `/root/.openclaw/openclaw.json`. Tools: xurl (X/Twitter CLI), clawhub (13,729 community skills)
- **clawd-worker:** AI DOGE data processing slave on vps-main
- **News Lancashire LLM chain:** Mistral Small (primary) → Gemini 2.5 Flash (backup). Rate-limited via `llm_rate_limiter.py`.
...
### Phase History
All phases complete. See AIDOGE-MASTERPLAN.md and memory/phase_history.md for detailed history.
- **Phases 1-18f** (Feb 2026): 15 councils live, auth, strategy, elections, intelligence, maps, property
- **v6-v7 + Cabinet Command** (28 Feb - 15 Mar): Councillor dossiers, directorate architecture, 62 engine functions, deep service intelligence (SEND/ASC/highways/waste), spending intelligence, ecosystem integration
- **Data Enhancement A-F** (9-16 Mar): Housing, Crime, Health, Economy pages + Census expansion
- **Statement of Accounts** (16 Mar): Loss trajectory, bond analysis, delivery weighting. 3,020 tests
- **Codebase Refactoring** (18 Mar): savingsEngine split into 15 modules, AbortController, ErrorState, useClipboard, chartDefaults, global test mocks. Commit `e4f0efa2`
