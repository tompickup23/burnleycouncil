...
- **Councillor Profiling:** `councillor_research_etl.py` → `councillor_profiles.json` — DOB, occupation, biography, structured employment/land/securities, committee memberships, electoral history. Run for all 15 councils (691 profiles)
- **FOI Generation:** `foi_generator.py` — auto-generate FOI requests from DOGE anomalies + integrity findings
...
- **Cabinet Command:** `cabinet_portfolios.json` → `savingsEngine.js` → Executive/DirectorateDashboard/DirectorateDetail/CabinetDashboard/PortfolioDetail pages. 5 directorates (matching LCC management structure) with evidence-backed savings, KPI→savings chains, 54 engine functions. 10 LCC portfolios with budget mapping, savings levers, statutory duties, governance routing. Prescriptive ACTION DIRECTIVES: DO/SAVE/LEGAL/ROUTE/HOW/EVIDENCE format. Deep Service Intelligence: SEND cost cascade (EHCP→placement→transport→tribunal), ASC demand model (demographics→care costs→market risk), cross-cutting intelligence (demand quantification, budget realism, inspection remediation, net fiscal trajectory). Spending Intelligence: 13-category classification, `useSpendingSummary` singleton hook, cross-platform spending access (Budgets, DOGE, Home, Executive, CrossCouncil, DirectorateDashboard, DirectorateDetail, PortfolioDetail). LCC-only (gated by `cabinet_portfolios` config flag).
...
| File | Purpose |
|------|---------|
| `src/App.jsx` | Router with 44 lazy-loaded routes |
| `src/pages/` | 42 page components + 55 test files (Spending, Budgets, DOGE, News, Elections, Constituencies, MPComparison, Integrity, Intelligence, Strategy, CouncillorDossier, PropertyPortfolio, PropertyDetail, LGRTracker, Demographics, Highways, Roadworks, Housing, Crime, Health, Economy, Executive, DirectorateDashboard, DirectorateDetail, CabinetDashboard, PortfolioDetail, etc.) |
...
| `src/hooks/useSpendingSummary.js` | Singleton spending summary hook — computes once per session, cached at module level. 13-category classification, portfolio aggregation, top suppliers, monthly trends. Any page can import |
...
| `src/components/pdf/PDFDesignSystem.js` | PDF design foundation: dark theme colors, typography, spacing, A4 page, 40+ styles, 8 utility functions |
| `src/components/pdf/PDFComponents.jsx` | 20+ shared PDF components: headers, footers, stat cards, tables, charts (pure View/Text), cover pages, ward issues |
| `src/components/pdf/CanvassingSheetPDF.jsx` | 2pp per-ward canvassing sheet + AllWardsCanvassingPDF. Ward-specific article intelligence (Hapton waste, Trinity HMOs, etc.) |
| `src/components/pdf/StrategistSheetPDF.jsx` | 5pp per-ward comprehensive strategist briefing (electoral history, demographics, attack lines, messaging) |
...
| `src/utils/savingsEngine.js` | Cabinet Command engine: centralised savings model, portfolio-spending mapping, prescriptive directives (DO/SAVE/LEGAL/ROUTE), decision pathways, reform playbooks, MTFS comparison, political impact, benchmarking. Directorate layer: buildDirectorateSavingsProfile, evidenceChainStrength, directorateKPITracker, benchmarkDirectorate, directorateRiskProfile. Tier 2: integrity-weighted HHI, CIPFA resilience, Benford screening, policy-area tagging, council attack lines, ward entrenchment, portfolio risk dashboard. Politics engine: Reform PR narrative (reformNarrativeEngine), electoral ripple (electoralRippleAssessment), borough election awareness, scrutiny premium, ward archetype targeting, REFORM_REBUTTALS matching, constituency resonance. Deep Service Intelligence: sendCostProjection, earlyInterventionROI, lacPlacementOptimisation, sendServiceDirectives, ascDemandProjection, ascMarketRisk, chcRecoveryModel, ascServiceDirectives, quantifyDemandPressures, budgetRealismCheck, inspectionRemediationTimeline, netFiscalTrajectory, highwayAssetTrajectory, wasteDisposalComparison, assetServiceDirectives, highwaysIntelligenceSummary, fiscalSystemOverview (54 functions, 366 tests). Spending Intelligence: spendingBudgetVariance, spendingConcentration, matchSpendingToPortfolio (enhanced summary mode) |
...
| `src/**/*.test.{js,jsx}` | Unit tests: 2,915 tests across 55 files (vitest) |
...
| `burnley-council/scripts/councillor_research_etl.py` | Councillor profiling ETL → councillor_profiles.json (DOB, occupation, committees, employment, electoral history) — run for all 15 councils (691 profiles) |
| `burnley-council/scripts/foi_generator.py` | Auto-generate FOI requests from DOGE anomalies + integrity findings → foi_templates.json |
...
| `councillor_profiles.json` | councillor_research_etl.py | Councillor profiling data (DOB, occupation, committees, employment, electoral history) — all 15 councils |
...
- **Ecosystem Engine Integration** (done, 15 Mar): 5-tier wiring of 42 engine functions into 14 pages. Tier 1: Meetings intelligence (policy areas, attack/defence lines, past votes). Tier 2: Budgets analytics (CIPFA, reserves, financial health). Tier 3: Talking points distributed to Housing/Health/Economy/Crime/MyArea/Demographics. Tier 4: Constituencies GE predictions, PropertyPortfolio LGR rationalisation. Tier 5: Home fiscal alert, Integrity HHI. 3 engine upgrades (savingsEngine forensics, electionModel integrity, lgrModel directives). +1,623 lines, 2,647 tests.
- **Phases 19-22** (done, 15 Mar): Phase 19: DogeInvestigation forensic surface (Benford 2nd digit, Gini gauge, peer benchmarks, fiscal stress), CrossCouncil analytical benchmarking + northern mill town comparison. Phase 20: LGR Tracker 8-function activation (CT harmonisation, threat assessment, equal pay risk, collection rate, CCA transfers, deprivation-adjusted savings, planning consolidation). Phase 21: Cross-engine bridges (intelligenceEngine←analytics fiscal context, strategyEngine←savingsEngine fiscal context). Phase 22: MyArea ward hub dashboard (glass-morphism summary cards, responsive grid, SPA navigation). +2,246 lines, 2,696 tests (53 files).
- **Cabinet Command v2** (done, 15 Mar): Savings-first directorate architecture. cabinet_portfolios.json enriched with 5 directorates, 35 performance_metrics with savings_links, evidence chains on 32/47 levers (data_points, benchmark, calculation, kpi_link, implementation_steps, political_framing), 3 new levers. savingsEngine.js +5 functions (buildDirectorateSavingsProfile, evidenceChainStrength, directorateKPITracker, benchmarkDirectorate, directorateRiskProfile), now 34 functions, 168 tests. DirectorateDashboard.jsx (replaces CabinetDashboard at /cabinet — 3 tabs: Command Centre, Directorates, MTFS Tracker, 21 tests). DirectorateDetail.jsx (/directorate/:id — 5 auth-gated tabs: Savings Overview, Performance, Savings Detail, Political Impact, Action Plan, 32 tests). Layout nav renamed to "Savings Command". Old CabinetDashboard preserved at /cabinet/old. 10 files, ~2,200 new lines. 2,646 tests (55 files).
- **Deep Service Intelligence Phases 1-3** (done, 15 Mar): Service-specific cost models for SEND/ASC + cross-cutting intelligence. Phase 1 (SEND): send_cost_model + lac_cost_model in cabinet_portfolios.json (EHCP pipeline, 7 placement types, tribunals, transport, DSG deficit, workforce, early intervention). 4 engine functions (sendCostProjection, earlyInterventionROI, lacPlacementOptimisation, sendServiceDirectives). SEND Cost Cascade UI in PortfolioDetail Overview tab (pipeline stats, placement pyramid, 5yr projection, transport levers, intervention ROI, LAC optimisation, workforce). Phase 2 (ASC): asc_demand_model + inspection_remediation in cabinet_portfolios.json (demographics, 8 care types, demand pressures, CHC, market sustainability, reablement). 4 engine functions (ascDemandProjection, ascMarketRisk, chcRecoveryModel, ascServiceDirectives). ASC Demand Dashboard UI (demographic pressure, market sustainability, CHC pipeline, reablement). Phase 3 (Cross-Cutting): 4 engine functions (quantifyDemandPressures, budgetRealismCheck, inspectionRemediationTimeline, netFiscalTrajectory). generateDirectives() Phase 5 auto-invokes service directives. 6 files, ~2,800 new lines, 111 new tests. 2,800 tests (55 files).
- **Deep Service Intelligence Phases 4-5** (done, 15 Mar): Phase 4 (Highways+Waste): highway_asset_model (condition trends, lifecycle costs, managed service, s59, LED, DfT allocation) + waste_model (disposal costs, landfill tax trajectory, EfW procurement, food waste mandate, market concentration) in cabinet_portfolios.json. 4 engine functions (highwayAssetTrajectory, wasteDisposalComparison, assetServiceDirectives, highwaysIntelligenceSummary). PortfolioDetail UI: highway backlog chart, road condition vs national, savings levers; waste disposal scenarios, landfill tax trajectory, market & mandates. Phase 5 (Unified Dashboard): fiscalSystemOverview engine function. DirectorateDashboard Fiscal System section (coverage badges, inspection status, demand vs savings chart). PortfolioDetail service intelligence summary card (demand growth, trajectory, coverage, breakeven). 9 files, +1,530 lines, 53 new tests. 2,853 tests (54 files).
- **Spending Intelligence Upgrade** (done, 15 Mar): 7-phase upgrade wiring LCC spending data across entire platform. 3-tier classification engine (13 canonical categories: dept patterns → expenditure keywords → heuristics). `useSpendingSummary` singleton hook (module-level cache, worker computes once per session). Worker `COMPUTE_SUMMARY` handler + `spend_category` filter + CSV column. Spending.jsx category treemap/filter/table column. savingsEngine +3 functions (spendingBudgetVariance, spendingConcentration, matchSpendingToPortfolio summary mode). Cross-platform integration: Budgets (actual vs budget variance), DOGE (category forensics), Home (spending banner), Executive (portfolio spend), CrossCouncil (category grid), DirectorateDashboard/DirectorateDetail/PortfolioDetail spending sections. 24 files, +2,073 lines, 62 new tests. 2,915 tests (55 files).
