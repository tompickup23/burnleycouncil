...
- **Cabinet Command:** `cabinet_portfolios.json` → `savingsEngine.js` → Executive/DirectorateDashboard/DirectorateDetail/CabinetDashboard/PortfolioDetail pages. 5 directorates (matching LCC management structure) with evidence-backed savings, KPI→savings chains, 51 engine functions. 10 LCC portfolios with budget mapping, savings levers, statutory duties, governance routing. Prescriptive ACTION DIRECTIVES: DO/SAVE/LEGAL/ROUTE/HOW/EVIDENCE format. Deep Service Intelligence: SEND cost cascade (EHCP→placement→transport→tribunal), ASC demand model (demographics→care costs→market risk), cross-cutting intelligence (demand quantification, budget realism, inspection remediation, net fiscal trajectory). LCC-only (gated by `cabinet_portfolios` config flag).
...
| `src/**/*.test.{js,jsx}` | Unit tests: 2,853 tests across 54 files (vitest) |
