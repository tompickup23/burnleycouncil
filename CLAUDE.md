...
8. **supplier_profiles.json files are huge** (~15MB/629K lines each) — only Rossendale's is committed to git. Suppliers nav gated by `supplier_profiles` data source flag in config.json
9. **Don't commit .json data files casually** — spending.json, supplier_profiles.json, doge_findings.json etc. are large generated files. Only commit when data has actually changed. spending-index.json, spending-YYYY-YY.json, and spending-YYYY-MM.json are gitignored (generated by ETL).
