...
4. **Builds must be sequential** — the vite plugin copies data to shared `public/data/`
5. **config.json is the source of truth** for council features (what pages show in nav, etc.)
6. **Data in public/ is ephemeral** — copied from burnley-council/data/ at build time, gitignored
7. **No API keys in code** — use environment variables
8. **Clawdbot config lives on vps-main only** — at `/opt/clawdbot/` and `/root/clawd/`. No agent config files in this repo.
9. **supplier_profiles.json files are huge** (~15MB/629K lines each) — only Rossendale's is committed to git. Suppliers nav gated by `supplier_profiles` data source flag in config.json
10. **Don't commit .json data files casually** — spending.json, supplier_profiles.json, doge_findings.json etc. are large generated files. Only commit when data has actually changed. spending-index.json, spending-YYYY-YY.json, and spending-YYYY-MM.json are gitignored (generated by ETL).
11. **Test builds before committing** — `VITE_COUNCIL=burnley VITE_BASE=/ npx vite build` should exit 0
...
- **0 articles**: All 6 new councils have empty articles-index.json
