...
- **Cabinet Command:** `cabinet_portfolios.json` → `savingsEngine.js` → Executive/DirectorateDashboard/DirectorateDetail/CabinetDashboard/PortfolioDetail pages. 5 directorates (matching LCC management structure) with evidence-backed savings, KPI→savings chains, 34 engine functions. 10 LCC portfolios with budget mapping, savings levers, statutory duties, governance routing. Prescriptive ACTION DIRECTIVES: DO/SAVE/LEGAL/ROUTE/HOW/EVIDENCE format. LCC-only (gated by `cabinet_portfolios` config flag).
...
| `src/**/*.test.{js,jsx}` | Unit tests: 2,646 tests across 55 files (vitest) |
