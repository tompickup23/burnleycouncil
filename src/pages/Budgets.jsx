...
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, LineChart, Line, PieChart, Pie, Cell, AreaChart, Area, Brush } from 'recharts'
    ...
    <div className="budgets-page animate-fade-in" aria-live="polite" aria-busy={loading}>
      ...
      {activeTab === 'revenue' && (
        <div className="tab-content">
            ...
            <div className="chart-card">
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={revenueChartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                  ...
                  {revenueChartData.length > 6 && (
                    <Brush dataKey="year" height={28} stroke="rgba(10,132,255,0.4)" fill="rgba(28,28,30,0.8)" travellerWidth={8} />
                  )}
                </BarChart>
              </ResponsiveContainer>
            </div>
          ...
          {/* Funding Waterfall — revenue build-up */}
          {fundingChartData.length >= 2 && selectedBudget?.net_revenue_budget && (
            <section className="chart-section">
              <h2>Revenue Budget Build-Up</h2>
              <p className="section-note">How the {selectedBudget.financial_year || 'current'} budget is funded</p>
              <div className="chart-card">
                <WaterfallChart
                  data={[
                    ...fundingChartData.map(f => ({
                      name: f.name,
                    ...
                    {
                      name: 'Net Budget',
                      value: selectedBudget.net_revenue_budget,
                      type: 'total',
                    },
                  ]}
          ...
          {/* Reserves Trajectory + Gauge — for auto-generated councils with reserves data */}
          {isAutoGenerated && budgetData?.reserves_trajectory?.length > 1 && (
            <section className="chart-section">
              ...
              {(() => {
                const latest = budgetData.reserves_trajectory[budgetData.reserves_trajectory.length - 1]
                const totalReserves = (latest.earmarked || 0) + (latest.unallocated || 0)
                const latestBudget = selectedBudget?.net_revenue_budget || 1
                const monthsOfSpend = latestBudget > 0 ? (totalReserves / latestBudget * 12) : 0
                return (
                  <div className="gauge-grid" style={{ marginBottom: 16 }}>
                    <GaugeChart
                      value={Math.min(monthsOfSpend, 12)}
                      max={12}
                      label="Months of Spend"
                      subtitle={`£${(totalReserves / 1_000_000).toFixed(1)}M total`}
                      format={(v) => v.toFixed(1)}
                      size={130}
                    />
                    <GaugeChart
                      value={(latest.unallocated || 0) / 1_000_000}
                      max={totalReserves / 1_000_000}
                      label="Unallocated"
                      subtitle={`£${((latest.unallocated || 0) / 1_000_000).toFixed(1)}M`}
