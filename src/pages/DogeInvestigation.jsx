...
import {
  ...
  Clock, Zap, Activity, ShieldAlert, Target, Gavel
} from 'lucide-react'
import {
  ...
  return (
    <div className="doge-page animate-fade-in">
      ...
      {documentsData?.decisions?.length > 0 && (
        <section className="doge-decisions-section" style={{ marginBottom: '32px' }}>
          <h2 style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <Gavel size={20} /> Related Council Decisions
          </h2>
          <p className="section-intro">
            Council decisions that relate to spending, procurement, or financial oversight.
          </p>
          <div style={{ display: 'grid', gap: '8px' }}>
            {documentsData.decisions
              .filter(d => {
                const areas = d.policy_areas || []
                return areas.includes('budget_finance') || areas.includes('council_tax') ||
                  areas.includes('procurement') || areas.includes('governance')
              })
              .slice(0, 10)
              .map((d, i) => (
                <div key={i} style={{
                  ...
                  {d.political_summary && (
                    <div style={{ fontSize: '0.75rem', color: '#888', marginTop: '4px', lineHeight: 1.5 }}>
                      {d.political_summary}
                    </div>
                  )}
                </div>
      ...
      {demoFiscalData && demoFiscalData.fiscal_resilience_score != null && (
        <ExpandableSection
          title="Demographic Fiscal Pressure"
          subtitle={`Fiscal Resilience: ${demoFiscalData.fiscal_resilience_score}/100 | Service Demand: ${demoFiscalData.service_demand_pressure_score}/100`}
          severity={demoFiscalData.fiscal_resilience_score < 30 ? 'critical' : demoFiscalData.fiscal_resilience_score < 50 ? 'warning' : 'info'}
        >
          <div className="analysis-content">
            <p className="section-methodology">
              Composite fiscal sustainability analysis combining ethnic demographic composition, SEND demand estimates,
              asylum dispersal costs, council tax yield, employment rates, and deprivation. Methodology: DfE SEND Statistics 2023,
              ONS Census 2021, Home Office Immigration Statistics.
            </p>

            {/* Score gauges */}
            <div className="fraud-triangle-scores">
              <div className="fraud-triangle-dim" style={{ borderColor: demoFiscalData.fiscal_resilience_score < 30 ? '#ff453a' : demoFiscalData.fiscal_resilience_score < 50 ? '#ff9f0a' : '#30d158' }}>
                <div className="dim-header">
                  <span className="dim-name">Fiscal Resilience</span>
                  <span className="dim-score" style={{ color: demoFiscalData.fiscal_resilience_score < 30 ? '#ff453a' : demoFiscalData.fiscal_resilience_score < 50 ? '#ff9f0a' : '#30d158' }}>
                    {demoFiscalData.fiscal_resilience_score}/100
                  </span>
                </div>
                <div className="dim-bar">
                  <div className="dim-fill" style={{ width: `${demoFiscalData.fiscal_resilience_score}%`, background: demoFiscalData.fiscal_resilience_score < 30 ? '#ff453a' : '#ff9f0a' }} />
                </div>
              </div>
              <div className="fraud-triangle-dim" style={{ borderColor: demoFiscalData.service_demand_pressure_score > 70 ? '#ff453a' : demoFiscalData.service_demand_pressure_score > 50 ? '#ff9f0a' : '#30d158' }}>
                <div className="dim-header">
                  <span className="dim-name">Service Demand Pressure</span>
                  <span className="dim-score" style={{ color: demoFiscalData.service_demand_pressure_score > 70 ? '#ff453a' : demoFiscalData.service_demand_pressure_score > 50 ? '#ff9f0a' : '#30d158' }}>
                    {demoFiscalData.service_demand_pressure_score}/100
                  </span>
                </div>
                <div className="dim-bar">
                  <div className="dim-fill" style={{ width: `${demoFiscalData.service_demand_pressure_score}%`, background: demoFiscalData.service_demand_pressure_score > 70 ? '#ff453a' : '#ff9f0a' }} />
                </div>
              </div>
            </div>
            ...
            <div style={{ marginTop: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <span className={`risk-badge ${demoFiscalData.risk_category === 'Structurally Deficit' ? 'critical' : demoFiscalData.risk_category === 'At Risk' ? 'warning' : 'info'}`}>
                {demoFiscalData.risk_category}
              </span>
            </div>
            ...
            {demoFiscalData.threats?.length > 0 && (
              <div style={{ marginTop: '1rem' }}>
                <h4 style={{ fontSize: '0.875rem', marginBottom: '0.5rem' }}>Key Threats</h4>
                <div className="finding-grid" style={{ gap: '0.5rem' }}>
                  {demoFiscalData.threats.slice(0, 4).map((t, i) => (
                    <div key={i} className="stat-pill" style={{ borderLeft: `3px solid ${t.severity === 'critical' ? '#ff453a' : t.severity === 'high' ? '#ff9f0a' : '#ffd60a'}`, padding: '0.5rem 0.75rem' }}>
                      <div style={{ fontSize: '0.8125rem', fontWeight: 500 }}>{t.description}</div>
            ...
            {demoFiscalData.send_risk && (
              <div style={{ marginTop: '1rem' }}>
                <h4 style={{ fontSize: '0.875rem', marginBottom: '0.5rem' }}>SEND Exposure</h4>
                <div className="stat-summary-row">
                  <div className="stat-pill info"><strong>{formatPercent(demoFiscalData.send_risk.estimated_send_rate_pct / 100)}</strong> estimated SEND rate</div>
                  {demoFiscalData.send_risk.cost_premium_pct > 0 && (
                    <div className="stat-pill warning"><strong>+{demoFiscalData.send_risk.cost_premium_pct.toFixed(1)}%</strong> cost premium vs average</div>
                  )}
                </div>
              </div>
            )}
            ...
            {demoFiscalData.pressure_zones?.length > 0 && (
              <div style={{ marginTop: '1rem' }}>
                <h4 style={{ fontSize: '0.875rem', marginBottom: '0.5rem' }}>Pressure Zone Wards</h4>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                  {demoFiscalData.pressure_zones.slice(0, 6).map((z, i) => (
                    <span key={i} className={`threshold-badge ${z.flag === 'CRITICAL' ? 'below' : 'meets'}`} style={{ fontSize: '0.75rem' }}>
                      {z.ward} <span style={{ opacity: 0.7 }}>(IMD dec. {z.imd_decile})</span>
                    </span>
                  ))}
                </div>
              </div>
            ...
            <div style={{ marginTop: '1rem', display: 'flex', gap: '1rem', fontSize: '0.8125rem' }}>
              <Link to="/people-and-communities" className="link-arrow">People & Communities → Fiscal Outlook</Link>
              <Link to="/lgr" className="link-arrow">LGR Tracker → Demographic Risk</Link>
            </div>

            <VerificationPanel checks={[
              { check: 'SEND rates from DfE SEND Statistics 2023 (by ethnicity)', status: 'pass' },
              { check: 'Fiscal score based on collection rates, employment, IMD, qualifications', status: 'pass' },
              { check: 'Asylum data from Home Office Immigration Statistics', status: 'pass' },
            ]} />
          </div>
        </ExpandableSection>
      )}
