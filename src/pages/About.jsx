...
function About() {
  ...
  const publisherSocial = config.publisher_social || []
  ...
  return (
    <div className="about-page animate-fade-in">
      ...
      <section className="intro-section">
        <div className="intro-content">
          <p>
            ...
            By law, they publish details of this spending — but it is typically released in spreadsheets and
            PDFs that can be difficult to navigate.
          </p>
          <p>
      ...
      {/* Creator Section — driven by config */}
      {publisher && (
        <section className="creator-section">
          <div className="creator-card">
            {config.publisher_photo && (
              <div className="creator-image">
                <img src={config.publisher_photo} alt={publisher} />
              </div>
            )}
            <div className="creator-content">
              <h2>Created by {publisher}</h2>
              {config.publisher_titles?.length > 0 && (
                <div className="creator-titles">
                  {config.publisher_titles.map((title, i) => (
                    <span key={i} className="title-badge">{title}</span>
                  ))}
                </div>
              )}
              {config.publisher_quote && (
                <blockquote className="creator-quote">
                  &ldquo;{config.publisher_quote}&rdquo;
                </blockquote>
              )}

              {publisherSocial.length > 0 && (
                <div className="social-links">
                  {publisherSocial.map(({ platform, url, label }) => (
                    <a
                      key={platform}
                      href={url}
      ...
      <section className="data-section" aria-label="Data sources">
        ...
        <div className="data-grid">
          <div className="data-card">
            <h4>Spending Data</h4>
            <p>
              Payments over £{spendingThreshold.toLocaleString()} and purchase card transactions,
              published quarterly under the Local Government Transparency Code.
              Procurement notices are sourced separately from Contracts Finder.
            </p>
        ...
        <div className="data-warning">
          <strong>Automated analysis:</strong> Where this site highlights patterns or anomalies in
          spending data, these are generated by automated processes. They are indicators that may
          warrant further review — not findings of wrongdoing or mismanagement.
        </div>
      </section>
      ...
      <section className="disclaimer-section" aria-label="Disclaimers and legal information">
        ...
        <div className="disclaimer-content">
          ...
          <div className="disclaimer-item">
            ...
            <p>
              This website has no connection to any political party and is not funded or
              directed by any party or campaign group.
            </p>
          </div>
