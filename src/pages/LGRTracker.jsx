...
function LGRTracker() {
  ...
  const sectionGroups = [
    ...
    { group: 'Local Impact', items: [
      { id: 'council-tax', label: 'Council Tax', icon: PoundSterling },
      { id: 'ct-harmonisation', label: 'CT Timeline', icon: Scale },
      ...
      { id: 'contract-transition', label: 'Contracts', icon: Briefcase },
    ]},
    { group: 'Risk Analysis', items: [
      { id: 'risks', label: 'Key Risks', icon: AlertTriangle },
      { id: 'timeline-risk', label: 'Timeline', icon: Clock },
      { id: 'implementation-risk', label: 'Implementation', icon: Target },
      { id: 'demographic-risk', label: 'Demographics', icon: Users },
      { id: 'deprivation', label: 'Deprivation', icon: BarChart3 },
      { id: 'revenue-impact', label: 'Revenue', icon: Zap },
      { id: 'efficiency-adjustments', label: 'Efficiency', icon: Activity },
    ]},
    { group: 'Evidence', items: [
      { id: 'precedents', label: 'UK Precedents', icon: Shield },
    ...
    <div className="lgr-page animate-fade-in">
      ...
      <div className="lgr-context-banner">
        ...
        <span>
          ...
          proposed for reorganisation into new unitary authorities. {relevantModels[0]?.myAuthority ? (
            <>Under the most-discussed model, {councilName} would become part of <strong>{relevantModels[0].myAuthority.name}</strong>.</>
          ) : daysUntilClose !== null && daysUntilClose > 0 ? (
            <>Five proposals are under public consultation until {new Date(lgrData.meta.consultation_closes).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}.</>
          ) : (
            <>Five proposals were submitted for public consultation. A decision is expected in 2026.</>
          )}
      ...
      <nav className="lgr-section-nav" aria-label="LGR Tracker sections">
        {sectionGroups.map(g => (
          <div key={g.group} className="lgr-nav-group">
            <span className="lgr-nav-group-label">{g.group}</span>
            {g.items.map(s => (
