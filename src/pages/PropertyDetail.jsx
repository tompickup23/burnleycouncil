...
const PATHWAY_COLORS = {
  ...
  development_partnership: '#12B6CF',
              ...
              <span className="property-detail-value" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                {asset.tier && (
                  <Badge
                    ...
                    color={asset.tier === 'subsidiary' ? '#ff9f0a' : asset.tier === 'jv' ? '#bf5af2' : asset.tier === 'third_party' ? '#ff453a' : '#12B6CF'}
                    bg={`${asset.tier === 'subsidiary' ? '#ff9f0a' : asset.tier === 'jv' ? '#bf5af2' : asset.tier === 'third_party' ? '#ff453a' : '#12B6CF'}22`}
                  />
                )}
              </span>
            </div>
    ...
    <div>
      ...
      <div className="glass-card" style={{ padding: 'var(--space-lg)', marginBottom: 'var(--space-lg)' }}>
        ...
        <div style={{ display: 'flex', gap: 'var(--space-xl)', flexWrap: 'wrap' }}>
          <div>
            <div style={{ fontSize: '1.5rem', fontWeight: 700, color: '#12B6CF' }}>
        ...
        <div className="glass-card" style={{ padding: 'var(--space-lg)', marginBottom: 'var(--space-lg)' }}>
          ...
          <div style={{ display: 'flex', gap: 'var(--space-xl)', flexWrap: 'wrap', marginBottom: 'var(--space-sm)' }}>
            ...
            {disp.revenue_estimate_annual > 0 && (
              <div>
                <div style={{ fontSize: '1.6rem', fontWeight: 700, color: '#12B6CF' }}>
...
function NpvBar({ label, npv, maxNpv, color }) {
  ...
  return (
    <div style={{ marginBottom: '10px' }}>
      ...
      <div style={{ background: 'rgba(255,255,255,0.06)', borderRadius: '4px', height: '8px', overflow: 'hidden' }}>
        <div style={{ width: `${pct}%`, height: '100%', background: color || '#12B6CF', borderRadius: '4px', transition: 'width 0.4s ease' }} />
      </div>
    </div>
  )
}
  ...
  return (
    <div>
      ...
      {gb?.options && (
        <div className="glass-card" style={{ padding: 'var(--space-lg)', marginBottom: 'var(--space-lg)' }}>
          <h3 style={{ fontSize: '1rem', marginBottom: 'var(--space-sm)', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <BarChart3 size={16} style={{ color: '#12B6CF' }} /> Green Book Options Appraisal
          </h3>
          ...
          <div style={{
            background: `${GB_OPTION_COLORS[gb.preferred_option] || '#12B6CF'}18`,
            border: `1px solid ${GB_OPTION_COLORS[gb.preferred_option] || '#12B6CF'}44`,
            ...
            <div>
              ...
              <div style={{ fontSize: '1rem', fontWeight: 700, color: GB_OPTION_COLORS[gb.preferred_option] || '#12B6CF' }}>
          ...
          {(() => {
            ...
            return opts.map(opt => (
              <NpvBar
                ...
                npv={opt.npv}
                maxNpv={maxAbs}
                color={GB_OPTION_COLORS[Object.keys(gb.options).find(k => gb.options[k] === opt)] || '#12B6CF'}
              />
            ))
          })()}
      ...
      {gb?.options && Object.entries(gb.options).sort(([,a],[,b]) => a.rank - b.rank).map(([code, opt]) => (
        <div key={code} className="glass-card" style={{
          ...
          borderLeft: `3px solid ${GB_OPTION_COLORS[code] || '#12B6CF'}`,
        }}>
          ...
          <div className="property-detail-grid">
            ...
            {asset.ownership_detail.ch_number && (
              <div className="property-detail-row">
                ...
                <span className="property-detail-value">
                  <a href={`https://find-and-update.company-information.service.gov.uk/company/${asset.ownership_detail.ch_number}`} target="_blank" rel="noopener noreferrer" style={{ color: '#12B6CF' }}>
                    {asset.ownership_detail.ch_number}
                  </a>
                </span>
              </div>
...
function SalesEvidenceSection({ evidence }) {
  ...
  return (
    <div className="glass-card" style={{ padding: 'var(--space-lg)', marginBottom: 'var(--space-lg)' }}>
      <h3 style={{ fontSize: '1rem', marginBottom: 'var(--space-md)', display: 'flex', alignItems: 'center', gap: '6px' }}>
        <TrendingUp size={16} style={{ color: '#12B6CF' }} /> Sales Evidence
      </h3>
      <div style={{ overflowX: 'auto' }}>
        <table className="property-table">
          <thead>
            ...
            {evidence.map((ev, i) => (
              <tr key={i}>
                ...
                <td>
                  <Badge
                    label={ev.status || '-'}
                    color={ev.status === 'marketed' ? '#30d158' : ev.status === 'sold' ? '#12B6CF' : '#8e8e93'}
...
function LGRTab({ asset, lgrData }) {
  ...
  return (
    <div>
      <div className="glass-card" style={{ padding: 'var(--space-lg)', marginBottom: 'var(--space-lg)' }}>
        <h3 style={{ fontSize: '1rem', marginBottom: 'var(--space-sm)', display: 'flex', alignItems: 'center', gap: '6px' }}>
          <Landmark size={16} style={{ color: '#12B6CF' }} /> LGR Authority Assignment
        </h3>
      ...
      {asset.primary_option && (
        <div className="glass-card" style={{
          ...
          <Badge
            ...
            color={RECOMMENDATION_COLORS[asset.primary_option.charAt(0).toUpperCase() + asset.primary_option.slice(1)] || '#12B6CF'}
            bg={`${RECOMMENDATION_COLORS[asset.primary_option.charAt(0).toUpperCase() + asset.primary_option.slice(1)] || '#12B6CF'}20`}
          />
        </div>
      )}
