...
export function fiscalSystemOverview(portfolios, cabinetData) {
  ...
  return {
    ...
    // Statement of Accounts loss data (if available)
    loss_trajectory: cabinetData?.administration?.statement_of_accounts
      ? lossTrajectoryAnalysis(cabinetData.administration.statement_of_accounts)
      : null,
    bond_analysis: cabinetData?.administration?.treasury
      ? bondPortfolioAnalysis(cabinetData.administration.treasury)
      : null,
  }
}
...
export function bondPortfolioAnalysis(treasury) {
  ...
  // Opportunity cost: what would the £600M earn if invested in gilts/MMFs instead?
 ...
 * @returns {{ cumulative_total: number, annual_average: number, worst_year: Object, trend: string, by_year: Array, loss_categories: Object }}
 */
...
export function opaqueSpendingAnalysis(spendingSummary, portfolios) {
  ...
  const OPAQUE_KEYWORDS = /^(payment|invoice|sundry|misc|various|general|other|n\/a|none|see narrative|as per|-)$/i
  const EMPTY_DESC = /^$/
