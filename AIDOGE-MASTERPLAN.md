...
### Live Infrastructure (£0/month)
| Asset | Status | Records | Spend Tracked |
|-------|--------|---------|---------------|
| Burnley SPA | LIVE | 30,580 | £355M |
| Hyndburn SPA | LIVE | 29,802 | £211M |
| Pendle SPA | LIVE | 49,741 | £127M |
| **Total** | | **110,123** | **£693M** |

### What Each Council Has vs Should Have

| Feature | Burnley | Hyndburn | Pendle | Target |
|---------|---------|----------|--------|--------|
| Spending Explorer | YES | YES | YES | All |
| DOGE Findings | YES | YES (config) | YES (config) | Generate per-council |
| Budget Trends (GOV.UK) | YES | YES | YES | All |
| Hand-Curated Budgets | YES | NO | NO | Burnley only (too manual) |
| News/Articles | YES (27 articles) | NO articles written | NO articles written | **Write per council** |
| Politics | YES (45 councillors) | YES (config) | YES (config) | Verify data loaded |
| My Area | YES | YES (config) | YES (config) | Add postcode lookup |
| FOI Templates | YES (15 Burnley-specific) | NO (generic only) | NO (generic only) | **Write per council** |
| Companies House | 23.8% linked | 18.9% linked | 19.0% linked | Target 60%+ |
| Crime Data | YES (Dec 2025) | YES (Dec 2025) | YES (Dec 2025) | Monthly auto |
| Executive Pay Comparison | NO | NO | NO | **NEW — build this** |
...
### 4A. Executive Pay Comparison Page (NEW)
**What:** Cross-council comparison of senior officer pay, councillor allowances, pay multiples.
**Data Sources:**
- Pay Policy Statements (published annually, legal requirement)
- Statement of Accounts (senior officer remuneration note)
- Councillor allowances (published, Transparency Code)
**Why:** Residents care about this. Easy to surface, hard to argue against transparency.
**Implementation:** New data file `pay_data.json` per council, new page `PayComparison.jsx`.

### 4B. Cross-Council Comparison Dashboard (NEW)
**What:** Side-by-side metrics for all 3 councils.
**Metrics:** Cost per head, reserves %, debt ratio, collection rates, supplier concentration, duplicate rates, CH compliance, waste cost per household, leisure subsidy per head.
**Why:** The power of DOGE is comparison. Each council in isolation is just numbers — together they reveal who's efficient and who isn't.
**Data Source:** Already have most data in `cross_council_comparison.json` + revenue_trends.

### 4C. Supplier Deep Dive Pages (NEW)
**What:** Click any supplier → full profile page showing all payments, trend over time, CH data, cross-council presence, compliance flags.
**Why:** Journalists need this. "Show me everything about Liberata" is the #1 use case.
**Implementation:** Dynamic route `/supplier/:name`, renders from spending.json + taxonomy.json.

### 4D. Council-Specific FOI Templates
**What:** Pre-written, copy-paste FOI requests tailored to each council's issues.
**Why:** FOI is the action step. Without it, DOGE is just data. With FOI templates, it becomes an accountability tool.
**Implementation:** `foi_templates.json` per council (generated from research above).

### 4E. "What Changed?" Tracking
...
| **SPA deployment** | Thurinus or Claude Code | `build_council.sh` + git push | After SPA changes |
...
- Thurinus (Oracle free tier) has 24GB RAM, 4 ARM cores — massively overpowered for this workload
...
4. **Only use Claude Code for:** SPA coding, complex analysis design, research, deployment.
...
3. **Waste Disposal: Lancashire's Shared Crisis** — All 3 councils face same Whinney Hill closure
...
- [ ] Fix the 4 critical code bugs
- [ ] Parameterise hardcoded Burnley references in SPA
- [ ] Generate Hyndburn + Pendle `doge_findings.json` properly
- [ ] Write council-specific FOI templates for Hyndburn + Pendle
- [ ] Commit all uncommitted work

### This Week (Claude Code + Clawdbot)
- [ ] Tom: Register for CH API key and paste into Thurinus
- [ ] Extract DOGE analysis into standalone script
- [ ] Build Executive Pay Comparison page skeleton
- [ ] Write first 3 Hyndburn articles
- [ ] Write first 3 Pendle articles
- [ ] Deploy updated SPAs

### This Month
- [ ] Build Supplier Deep Dive pages
- [ ] Build Cross-Council Comparison dashboard
- [ ] Add postcode → ward lookup (postcodes.io)
- [ ] Set up Thurinus data monitoring script
- [ ] Set up councillor scraping
- [ ] Write remaining articles (22 total)
- [ ] Improve CH matching to 60%+ coverage

### Ongoing (Automated)
- Monthly: CH matching cron, police crime ETL, data freshness monitoring
...
|--------|---------|--------|-----|
| Councils live | 3 | 5+ | Add Rossendale, Ribble Valley |
| Articles per council | 27/0/0 | 27/8/8 | Write Hyndburn + Pendle articles |
| CH match rate | 20% | 60% | API key + fuzzy matching |
| Duplicate detection | Basic | Confidence-scored | Enhance algorithm |
| FOI templates | 15 (Burnley only) | 15 per council | Research + write |
| Monthly cost | £0 | £0 | Keep everything free |
| Credit usage | High (all via Claude Code) | Minimal | Delegate to Thurinus/Clawdbot |
| Data freshness | Manual | Auto-monitored | Thurinus monitoring script |
